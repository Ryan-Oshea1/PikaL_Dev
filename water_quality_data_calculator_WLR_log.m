...
%use Rrs off WLR to calculate WQP
   ...
   if(srg_correction == 1)
    ...
    p = polyfit(x,y,order);
    y1= polyval(p,x);
      figure; scatter(x,y); hold on; scatter(x,y1); xlabel('Ratio'); ylabel('predicted and actual'); legend('Actual conc', 'Predicted Conc')
...
% get statistics
%ccr = corrcoef(10.^forty_deg_red_reflectance_numer(not_nan_loc),(water_quality_parameter_values_chl(not_nan_loc)));
