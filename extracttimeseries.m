...
set_num_range = 7;
read_radiometer_data = 1;
folder_range = 1:(length(folders)-7);
...
set_num_path = [folderOfInterest folders(k).name HyperImage 'set_' num2str(set_num)];
...
if select_polarized_images == 1 && select_gain_images == 0
    set_num_path = [;folderOfInterest folders(k).name HyperImagePol 'set_' num2str(set_num)];
...
clear read_radiometer_data select_gain_images select_polarized_images set_num set_num_path set_num_range
clear k
