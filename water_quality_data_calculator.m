...
%chl_or_beta == 1, is chl, 2 is beta
index = 1;

for ( angle_increment =1:225)
angle_increment
    ...
    %set the different wavelength ranges
...
%                                         [err, pgxd_0, rrs_mod, rrs_mea, bbp_650] = rrs_optimization_func(minimization_parameters,wavelengths_rrs_opt,trs,srs,aph_const,aph_coef,aw,bbw,g_p_9,g_p_10,g_p_11,sdg,fresnel,gw,sub_to_above_1,sub_to_above_2,trs_wavelengths,srs_wavelengths);
                                           ...
                                           size(rrs_mea);
                                          if( chl_or_beta == 1) %chl algorithm
   ...
   if( chl_or_beta == 1) %chl algorithm
    ...
    y = log10((water_quality_parameter_values(not_nan_loc))');  %actual
    x = log10(forty_deg_red_reflectance_numer(not_nan_loc)'); %predicted
    p = polyfit(x,y,order);
    y1= polyval(p,x);
...
%     y = ((water_quality_parameter_values(not_nan_loc))');  %actual
%     x = (forty_deg_red_reflectance_numer(not_nan_loc)'); %predicted
%     p = polyfit(x,y,order);
%     y1= polyval(p,x);
...
not_nan_loc = unique(find(not_nan_loc_finder)); 

 not_nan_loc = not_nan_loc';
 not_nan_loc_loc = find(not_nan_loc);
 not_nan_loc = not_nan_loc(not_nan_loc_loc);
...
% not_nan_loc_holder(index).value = not_nan_loc;
% 
% [ccr, p_values] = corrcoef(predicted_beta,(water_quality_parameter_values(not_nan_loc)));
% 
% corr_coeff_ratio(index) = ccr(1,2);
% p_value = p_values(1,2);
...
predicted_beta_holder(index).value = predicted_beta;
not_nan_loc_holder(index).value = not_nan_loc;
[ccr, p_values] = corrcoef(predicted_beta,(water_quality_parameter_values(not_nan_loc)))
